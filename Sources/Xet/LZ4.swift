...
public enum LZ4 {
    ...
    /// Decompresses an LZ4 block directly into a provided buffer.
    ///
    /// This variant avoids allocating a new `Data` by writing directly
    /// into the caller's buffer. Handles both raw blocks and standard frames.
    ///
    /// - Parameters:
    ///   - compressed: The compressed block data (raw or framed).
    ///   - destination: Buffer to write decompressed data into.
    ...
    public static func decompressBlock(
        ...
        // Check for standard LZ4 frame magic bytes (little-endian 0x184D2204)
        if compressed.count >= 4,
            let base = compressed.bindMemory(to: UInt8.self).baseAddress,
            base[0] == 0x04,
            base[1] == 0x22,
            ...
            base[3] == 0x18
        {
            // Standard frame - fall back to allocating path
            let data = Data(bytes: base, count: compressed.count)
            let framed = try decompressStandardFrame(data, expectedSize: maxOutputSize)
            guard framed.count == maxOutputSize else { throw LZ4Error.decompressionFailed }
            framed.withUnsafeBytes { src in
                if let srcBase = src.baseAddress {
                    memcpy(destination, srcBase, src.count)
                }
            }
            return framed.count
        }
        ...
        let fallbackResult = try decompressRawBlock(compressed, maxOutputSize: maxOutputSize)
        fallbackResult.withUnsafeBytes { src in
            if let srcBase = src.baseAddress {
                memcpy(destination, srcBase, src.count)
            }
        }
        return fallbackResult.count
    }
